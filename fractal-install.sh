...
download_and_extract() {
    local url="https://github.com/fractal-bitcoin/fractald-release/releases/download/v${VERSION}/fractald-${VERSION}-x86_64-linux-gnu.tar.gz"
    local filename="fractald-${VERSION}-x86_64-linux-gnu.tar.gz"
    local dirname="fractald-${VERSION}-x86_64-linux-gnu"
...
create_wallet() {
    ...
    cd fractald-${VERSION}-x86_64-linux-gnu/bin || handle_error "进入目录 bin 失败。"
...
[Service]
User=$USER
ExecStart=$HOME/fractald-${VERSION}-x86_64-linux-gnu/bin/bitcoind -datadir=$HOME/fractald-${VERSION}-x86_64-linux-gnu/data/ -maxtipage=504576000
Restart=always
RestartSec=5
